...
namespace Vaquinha.Integration.Tests
{
    ...
    public class HomeTests
    {
        ...
        [Trait("HomeControllerIntegrationTests", "HomeController_CarregarPaginaInicial_NenhumDoador")]
        [Fact]
        public async Task HomeController_CarregarPaginaInicial_NenhumDoador()
        {
            // Arrange & Act
            var home = await _integrationTestsFixture.Client.GetAsync("Home");
            ...
            home.EnsureSuccessStatusCode();
            var dadosHome = await home.Content.ReadAsStringAsync();
            dadosHome.Should().NotContain(unexpected: "Quem já doou?");
        }
    }
}